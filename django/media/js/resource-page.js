...
function getData(selector, getter){
    ...
    var s = '[' + $(selector).get().map(getter).join() + ']';
    //var list = [];
    return s;
}

function entries(url){
    var s = getData("tr.success");
    var data={};
    data.entries=s;
...
function button_edit_click() {
    getPositions({on_success: function(html){
            showPositions(html);
            //$(".well").show();
...
function button_export_click() {
        ...
        $(this).attr('href', $(this).data('url') + '?date_since=' + min + '&date_until=' + max+'&view='+getVar("view-id"));
}
